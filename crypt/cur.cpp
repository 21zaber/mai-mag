...
int main() {
    ...
    MAX_LEN = 16;
    HALF_LEN =  (MAX_LEN/2);
    MAX_VECTOR_COUNT = powers[MAX_LEN*2];

    srand(time(NULL));
    tStart = clock();

    //cout << "Memory allocation for " << MAX_VECTOR_COUNT / 32 << " vectors: ";
    ...
    matrix[0] = 0;//llrand();
    matrix[1] = 0;//llrand();
    for (int i = 0; i < quater_len; i++){
        matrix[1] += powers[quater_len*2+i*2];
        matrix[1] += powers[quater_len*2*2+i*2+1];
        matrix[1] += powers[quater_len*2*3+i*2];
        matrix[1] += powers[quater_len*2*3+i*2+1];
    }
  ...
  //matrix[2] += ((VECTOR_COUNTER)1 << (quater_len * 1)) - 1; 
    ...
    //cout << "Step 1. Current Current size of vector's list - " << p << endl;

    for (LENGTH_COUNTER step = 2; step < MAX_LEN; ++step) {
    ...
    for (int i = 0; i < MAX_LEN; ++i){
        inverse_1 += powers[i*2];
        inverse_i += powers[i*2+1];
    }

    VECTOR mask_1 = 1 << (MAX_LEN-1)*2;
    VECTOR mask_i = 2 << (MAX_LEN-1)*2;
